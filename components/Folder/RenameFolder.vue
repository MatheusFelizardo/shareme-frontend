<template>
  <div>
    <DialogRoot modal :open="props.showRenameFolder">
      <DialogPortal>
        <DialogOverlay
...
const folder = ref(props.folder.name);
const error = ref("");
const status = ref("")
const timeouts = ref([]);
...
const renameFolder = async () => {
  ...
  if (folder.value === oldName) {
    error.value = "Folder name is the same as the old name";
    isLoading.value = false;
    return;
  }

  const response = await $folderStore.renameFolder(props.folder.id, folder.value);
