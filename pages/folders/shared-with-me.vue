<template>
  <div class="pt-8 px-4 relative overflow-hidden h-full">
    ...
    <div v-else>
      <div v-if="sharedWithMe.length > 0" class="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4 mt-8 pb-4 overflow-y-auto max-h-[calc(100svh-130px)]">
        <div v-for="folder in sharedWithMe" :key="folder.id"  class="relative flex bg-gray-200 px-4 py-2 shadow-md justify-between overflow-hidden text-black">
          <router-link 
              class="w-full"
              :to="`/folders/${folder.id}?origin=shared_with_me`" 
            ...
            <div class="flex items-center gap-4 w-10/12">
              <div>
                ...
                <svg v-else focusable="false" viewBox="0 0 24 24" height="24px" width="24px" fill="rgb(234 179 8)" class=""><g><path d="M10 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V8c0-1.1-.9-2-2-2h-8l-2-2z"></path><path d="M0 0h24v24H0z" fill="none"></path></g></svg>
              </div>
      ...
      <div v-else class="flex mt-8 pb-6 pointer-events-none">
        <p>No folders shared with you.</p>
      </div>
    </div>
...
<script setup>
  ...
  const { sharedWithMe } = storeToRefs($folderStore)
  const isLoading = ref(true)

  onMounted(async () => {
    await $folderStore.foldersSharedWithMe()
    setTimeout(() => {
