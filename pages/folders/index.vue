<template>
  <div class="pt-8 px-4 relative overflow-hidden h-full">
    ...
    <div v-else>
      <div v-if="folders.length > 0" class="grid grid-cols-[repeat(auto-fill,minmax(220px,1fr))] gap-4 mt-8 pb-4 overflow-y-auto max-h-[calc(100svh-130px)]">
        
        <div v-for="folder in folders" :key="folder.id" :title="folder.name" class="relative flex bg-gray-200 px-4 py-2 shadow-md justify-between overflow-hidden text-black">
          <router-link 
              class="w-full"
          ...
          <div class="absolute right-2 cursor-pointer">
            <FolderDropdown 
              :folder="folder" 
              :open="oppenedDropdown == folder.id" 
              :setOpen="() => oppenedDropdown = folder.id"
              :close="() => oppenedDropdown = null"
            >
      ...
      <div v-else class="flex flex-col mt-8 pb-6">
        <p>You don't have any folder.</p>
        <p class="text-gray-500">
          <button @click="showCreateFolder = true" class="text-indigo-500">Click here</button> to create a new folder to start uploading files.</p>
        <CreateFolderModal :closeModal="()=> showCreateFolder = false" :open="showCreateFolder" />
      </div>
    </div>
  </div>
</template>
